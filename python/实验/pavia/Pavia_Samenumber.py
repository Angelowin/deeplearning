...
data = 'G:\machinelearning\python\实验\CGAN\Spectual.mat'
a = sio.loadmat(data)
data_tr = a['x_spe']
label_tr = a['s_label']

##############
ran = 0.2-0.4*np.random.random((1,103))
num = np.arange(103)
data_test = data_tr[30,:]
plt.xticks([])
plt.yticks([])
for i in range(9):
    plt.figure(1)
    da = np.array(data_tr[9*i+4,:])+ np.array(ran)
    plt.plot(num,da.T,linestyle = "--")

    #plt.plot(num,data_tr[1,:])
# data_new = np.array(data_tr[248,:])+np.array(ran)
# plt.plot(num,data_new.T,color = "red")
# data_new = np.array(data_tr[249,:])+np.array(ran)
# plt.plot(num,data_new.T,color = "red")
# data_new = np.array(data_tr[250,:])+np.array(ran)
# plt.plot(num,data_new.T,color = "red")
