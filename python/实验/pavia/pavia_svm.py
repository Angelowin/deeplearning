...
for a in range(1):
    ...
    for rand in range(1):
        label = sio.loadmat('/media/jiming/_E/angelo/ALofauto/PaviaU_gt.mat')[
            'paviaU_gt']
        data_500 = np.load(
            '/media/jiming/_E/angelo/Paper/degan_pavia/feature200_no30-5.npy')
        ...
        data = np.reshape(data_500, [300 * 300, 200], order='F')
        data_p = prep.scale(data)
    ...
    #############
        ...
        # label = np.reshape(label, [300 * 300, 1], order='F')
        # data_p = np.reshape(data_500, [300 * 300, 10], order='F')
    ...
    ##############
        ...
        for seed in range(1):
            ...
            # sorce_zeros = np.zeros(300*300)
            # for i in range(17423):
            #     sorce_zeros[index_x[i]] = p_label[i]
            #
            # label_new = np.reshape(sorce_zeros, [300,300], order='F')
            # label_new = label_new.astype(np.uint8)
            #
            # sio.savemat('g_pavia30_5',mdict={'g_pavia30':label_new})
