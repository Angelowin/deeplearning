...
def plot_confusion_matrix(y_true, y_pred, labels):
    ...
    for x_val, y_val in zip(x.flatten(), y.flatten()):
        if (intFlag):
            c = cm[y_val][x_val]
            plt.text(x_val, y_val, "%d" % (c,), color='black', fontsize=10, va='center', ha='center')
            ...
            if (c > 0.01):
                ...
                plt.text(x_val, y_val, "%d" % (0,), color='black', fontsize=10, va='center', ha='center')
    if (intFlag):
    ...
    # plt.ylabel(u'真实类别')
    # plt.xlabel(u'预测类别')
    plt.savefig('s_indian_new10-40.jpg', dpi=300)
    plt.show()
